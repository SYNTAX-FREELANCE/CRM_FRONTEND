...
const PolicyReport = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const response = await axioslogin.get(
                `/reports/policy?fromDate=${startDate}&toDate=${endDate}`
            );
            
            if (response.data?.success === 1) {
                setReportData(response.data.data || []);
                setPage(0);
                successNotify(`Retrieved ${response.data.data?.length || 0} records successfully.`);
            } else {
                warningNotify(response.data?.message || "Failed to fetch report data");
                setReportData([]);
            }
        } catch (error) {
            console.error("Fetch report error:", error);
            errorNotify("An error occurred while fetching the policy report");
            setReportData([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleExportExcel = async () => {
        ...
        try {
            ...
            const response = await axioslogin.get(
                `/reports/policy/export?fromDate=${startDate}&toDate=${endDate}`,
                { responseType: "blob" }
            );
            ...
            link.href = url;
            link.setAttribute("download", `policy_report_${startDate}_to_${endDate}.xlsx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
            window.URL.revokeObjectURL(url);
            successNotify("Excel sheet downloaded successfully!");
        } catch (error) {
            console.error("Export Excel error:", error);
            errorNotify("Failed to download Excel sheet");
        } finally {
                        ...
                        {loading ? (
                            ...
                            <Box sx={{ textAlign: "center", py: 8, border: isDark ? "2px dashed rgba(255,255,255,0.15)" : "2px dashed #cbd5e1", borderRadius: "16px" }}>
                                ...
                                <Typography level="body-sm" sx={{ color: textSecondaryColor, mt: 1 }}>
                                    No lead activities were logged between the selected dates. Try adjusting your parameters.
                                </Typography>
                            </Box>
                        ) : (
                            <Box>
                ...
                {!searched && (
                    ...
                    >
                        ...
                        <Typography level="body-sm" sx={{ color: textSecondaryColor, mt: 1, maxWidth: 400 }}>
                            Specify a date range and click search to dynamically construct the policy workflow audit report.
                        </Typography>
                    </Box>
                )}
            </Card>
